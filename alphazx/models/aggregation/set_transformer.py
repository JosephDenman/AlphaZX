...
from alphazx.models.aggregation.utils import PoolingByMultiheadAttention, SetAttentionBlock
...
class SetTransformerAggregation(Aggregation):
    ...
    def forward(
        ...
        # print('sta.x.shape = ', x.shape)
        # print('sta.mask.shape = ', mask.shape)

        for encoder in self.encoders:
            x = encoder(x, mask)

        # print('sta.pre_pma.x.shape = ', x.shape)

        x = self.pma(x, mask)

        # print('sta.post_pma.x.shape = ', x.shape)
