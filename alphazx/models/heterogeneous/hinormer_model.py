...
class AGTLayer(nn.Module):
    def __init__(self, embeddings_dimension, num_heads=2, att_dropout=0.5, emb_dropout=0.5, temper=1.0, rl=False,
        ...
        self.head_dim = self.embeddings_dimension // self.num_heads
        ...
        self.linear_l = nn.Linear(
            self.embeddings_dimension, self.head_dim * self.num_heads, bias=False)
        ...
        if rl:
            self.r_source = nn.Linear(rl_dim, rl_dim * self.num_heads, bias=False)
            self.r_target = nn.Linear(rl_dim, rl_dim * self.num_heads, bias=False)

        self.linear_final = nn.Linear(
            self.head_dim * self.num_heads, self.embeddings_dimension, bias=False)
    ...
    def forward(self, h, rh=None):
        ...
        if rh is not None:
            r_k = self.r_source(rh).reshape(batch_size, -1, self.num_heads, self.rl_dim).transpose(1, 2)
            r_q = self.r_target(rh).reshape(batch_size, -1, self.num_heads, self.rl_dim).permute(0, 2, 3, 1)
            score_r = r_k @ r_q
        ...
        h_sa = context.transpose(1, 2).reshape(batch_size, -1, self.head_dim * self.num_heads)
...
class HINormer(nn.Module):
    def __init__(self, g, num_class, input_dimensions, embeddings_dimension=64, num_layers=8, num_gnns=2, num_heads=2,
        ...
        for layer in range(self.num_layers):
            self.GTLayers.append(
                AGTLayer(self.embeddings_dimension, self.num_heads, self.dropout, self.dropout, temper=temper, rl=True,
