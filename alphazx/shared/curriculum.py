...
class CurriculumConfig:
    ...
    advance_threshold: float = -0.2
    """For 'performance' strategy: minimum average T-gate reduction ratio
    (avg_t_reduced / avg_initial_t) required to advance.

    NEGATIVE values allow advancement when the agent is still a net T-gate
    increaser but less bad than random play (which averages about -1.0 to
    -1.5 for small circuits).  This prevents the curriculum from getting
    stuck at level 0 forever while the agent is still learning.

    -0.2 means the agent can advance once it limits average T-gate increase
    to 20% of initial.  Set to 0.0 to require net-zero, or positive values
    to require actual reduction."""
...
class CurriculumScheduler:
    ...
    def _check_performance_advance(
        ...
        # Log the reduction ratio and threshold comparison every iteration
        # so the user can diagnose advancement failures.
        history_str = ', '.join(f'{r:.3f}' for r in self._performance_history)
        meets = sum(1 for r in self._performance_history if r >= self.config.advance_threshold)
        logger.info(
            f"Curriculum check: reduction_ratio={reduction_ratio:.3f} "
            f"(avg_reduced={avg_t_gates_reduced:.2f}, avg_initial={avg_initial_t_gates:.2f}), "
            f"threshold={self.config.advance_threshold}, "
            f"history=[{history_str}], "
            f"{meets}/{len(self._performance_history)} meet threshold "
            f"(need {self.config.advance_window})"
        )
