...
from src.diagram.constants import B_ETYPE_NAME, I_ETYPE_NAME
from src.diagram.match import Match, CompoundMatch, FRightMatch, FRightZMatch, FRightXMatch, BoundaryMatch, BaseMatch, \
...
class ZXMatchDiagram(nx.MultiDiGraph):
    ...
    # TODO: Include a top-level attribute on 'HeteroData' that maps each node in each type dictionary of HeteroData to the index
    #       of the node in the 'nodes' attribute of the input 'ZXDiagram' instance. The map should be dict[int, int], where the
    #       keys are the indices of the
    def to_pyg_hdata(self) -> pyg_data.HeteroData:
